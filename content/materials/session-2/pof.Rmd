...
prejudice_df %>%
  dplyr::mutate(ite = prej_1 - prej_0) %>%
  dplyr::summarize(mean(ite)) %>% knitr::kable() %>% kableExtra::kable_styling(bootstrap_options = "striped", full_width = F, position = "center")
```
...
The average treatment effect on the control **ATC** is defined as the difference in the average potential outcomes for those subjects who were not treated:
...
We have already extracted the ite with `mutate()`. We know that the ATT and ATU are the average of every subject's ITE grouped by their treatment status. Do you remember how the combination of `group_by()` and `summarize()` worked?
...
We know that the HTE bias is the difference in difference in the average of every subject's ITE grouped by their treatment status (or the difference between ATT and ATCs). Maybe you can go back an check how to get the average treatment effect on the treated and untreated.
